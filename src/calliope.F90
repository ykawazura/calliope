program calliope
  ...
  use params, only: init_params, nwrite, nwrite_fld_section, nwrite_fld_3D, nwrite_kpar, nwrite_SF2, nsave_restart
  ...
  use diagnostics, only: init_diagnostics, finish_diagnostics, loop_diagnostics, &
                         loop_diagnostics_fields_secion, loop_diagnostics_kpar, loop_diagnostics_SF2
  ...
  do istep = 1, nstep
    ...
    if(mod(istep, nwrite_kpar       ) == 0) call loop_diagnostics_kpar          ! output for kpar & delta b/b0 using k-filtering
