...
module io
  ...
  integer :: fh_aaa_r_z0, fh_aaa_r_x0, fh_aaa_r_y0
  ...
  integer (kind=MPI_OFFSET_KIND) :: disp_aaa_r_z0, disp_aaa_r_x0, disp_aaa_r_y0
  ...
  ! parameter
  integer :: nupe_id, nupe_exp_id, nuz_id, nuz_exp_id
  integer :: etape_id, etape_exp_id, etaz_id, etaz_exp_id
  integer :: chipe_id, chipe_exp_id, chiz_id, chiz_exp_id
  ! coordinate
  integer :: xx_id, yy_id, zz_id, kx_id, ky_id, kz_id, kpbin_id, tt_id
  ! total energy
  integer :: upe2_sum_id, bpe2_sum_id, aaa2_sum_id
  integer :: upe2dot_sum_id, bpe2dot_sum_id, aaa2dot_sum_id
  integer :: upe2dissip_sum_id, bpe2dissip_sum_id, aaa2dissip_sum_id
  integer :: p_sum_id
  integer :: zpep2_sum_id, zpem2_sum_id
  ! polar spectrum
  integer :: upe2_bin_id, bpe2_bin_id, aaa2_bin_id
  integer :: ux2_bin_id , uy2_bin_id , bx2_bin_id , by2_bin_id
  integer :: p_bin_id
  integer :: zpep2_bin_id, zpem2_bin_id
...
!-----------------------------------------------!
  subroutine init_io_decomp_3d
    ...
    call set_file_handle('out3d/aaa.dat', fh_aaa, disp_aaa)
...
!-----------------------------------------------!
  subroutine init_io_decomp_2d
    ...
    call set_file_handle('out2d/aaa_r_z0.dat', fh_aaa_r_z0, disp_aaa_r_z0)
    call set_file_handle('out2d/aaa_r_x0.dat', fh_aaa_r_x0, disp_aaa_r_x0)
    call set_file_handle('out2d/aaa_r_y0.dat', fh_aaa_r_y0, disp_aaa_r_y0)
...
!-----------------------------------------------!
  subroutine init_io_netcdf(nkpolar, kpbin)
    ...
    if(proc0) then
      ...
      status = nf90_def_var (ncid, 'nupe', NF90_DOUBLE, nupe_id)
      status = nf90_def_var (ncid, 'nupe_exp', NF90_DOUBLE, nupe_exp_id)
      status = nf90_def_var (ncid, 'nuz', NF90_DOUBLE, nuz_id)
      status = nf90_def_var (ncid, 'nuz_exp', NF90_DOUBLE, nuz_exp_id)
      status = nf90_def_var (ncid, 'etape', NF90_DOUBLE, etape_id)
      status = nf90_def_var (ncid, 'etape_exp', NF90_DOUBLE, etape_exp_id)
      status = nf90_def_var (ncid, 'etaz', NF90_DOUBLE, etaz_id)
      status = nf90_def_var (ncid, 'etaz_exp', NF90_DOUBLE, etaz_exp_id)
      status = nf90_def_var (ncid, 'chipe', NF90_DOUBLE, chipe_id)
      status = nf90_def_var (ncid, 'chipe_exp', NF90_DOUBLE, chipe_exp_id)
      status = nf90_def_var (ncid, 'chiz', NF90_DOUBLE, chiz_id)
      status = nf90_def_var (ncid, 'chiz_exp', NF90_DOUBLE, chiz_exp_id)
      ...
      status = nf90_def_var (ncid, 'aaa2_sum', NF90_DOUBLE, tt_dim, aaa2_sum_id)
      ...
      status = nf90_def_var (ncid, 'aaa2dot_sum', NF90_DOUBLE, tt_dim, aaa2dot_sum_id)
      ...
      status = nf90_def_var (ncid, 'aaa2dissip_sum', NF90_DOUBLE, tt_dim, aaa2dissip_sum_id)
      status = nf90_def_var (ncid, 'p_sum'   , NF90_DOUBLE, tt_dim, p_sum_id   )
      ...
      status = nf90_def_var (ncid, 'aaa2_bin', NF90_DOUBLE, bin_dim, aaa2_bin_id)
      ...
      status = nf90_def_var (ncid, 'p_bin'   , NF90_DOUBLE, bin_dim, p_bin_id   )
      ...
      status = nf90_put_var (ncid, nupe_id, nupe)
      status = nf90_put_var (ncid, nupe_exp_id, dble(nupe_exp))
      status = nf90_put_var (ncid, nuz_id, nuz)
      status = nf90_put_var (ncid, nuz_exp_id, dble(nuz_exp))
      status = nf90_put_var (ncid, etape_id, etape)
      status = nf90_put_var (ncid, etape_exp_id, dble(etape_exp))
      status = nf90_put_var (ncid, etaz_id, etaz)
      status = nf90_put_var (ncid, etaz_exp_id, dble(etaz_exp))
      status = nf90_put_var (ncid, chipe_id, chipe)
      status = nf90_put_var (ncid, chipe_exp_id, dble(chipe_exp))
      status = nf90_put_var (ncid, chiz_id, chiz)
      status = nf90_put_var (ncid, chiz_exp_id, dble(chiz_exp))
...
!-----------------------------------------------!
  subroutine loop_io( &
    ...
    implicit none
    real(r8), intent(in) :: upe2_sum, bpe2_sum, aaa2_sum
    real(r8), intent(in) :: upe2dot_sum, bpe2dot_sum, aaa2dot_sum
    real(r8), intent(in) :: upe2dissip_sum, bpe2dissip_sum, aaa2dissip_sum
    real(r8), intent(in) :: p_sum
    real(r8), intent(in) :: zpep2_sum, zpem2_sum

    integer, intent(in) :: nkpolar
    real(r8), intent(in) :: upe2_bin(1:nkpolar, nkz), bpe2_bin(1:nkpolar, nkz)
    real(r8), intent(in) :: aaa2_bin(1:nkpolar, nkz)
    ...
    real(r8), intent(in) :: p_bin   (1:nkpolar, nkz)
    ...
    if(proc0) then
      ...
      status = nf90_put_var (ncid, aaa2_sum_id, aaa2_sum, start=(/nout/))
      ...
      status = nf90_put_var (ncid, aaa2dot_sum_id, aaa2dot_sum, start=(/nout/))
      ...
      status = nf90_put_var (ncid, aaa2dissip_sum_id, aaa2dissip_sum, start=(/nout/))
      status = nf90_put_var (ncid, p_sum_id   , p_sum   , start=(/nout/))
      ...
      status = nf90_put_var (ncid, aaa2_bin_id, aaa2_bin, start=start3, count=count3)
      ...
      status = nf90_put_var (ncid, p_bin_id   , p_bin   , start=start3, count=count3)
...
!-----------------------------------------------!
  subroutine loop_io_2D( &
    ...
    real(r8), intent(in) :: aaa_r_z0(ilx_st:ilx_en, ily_st:ily_en), &
                            aaa_r_x0(ily_st:ily_en, ilz_st:ilz_en), &
                            aaa_r_y0(ilx_st:ilx_en, ilz_st:ilz_en)
    ...
    call mpiio_write_var_2d(fh_aaa_r_z0, disp_aaa_r_z0, sizes, subsizes, starts, aaa_r_z0)
    ...
    call mpiio_write_var_2d(fh_aaa_r_x0, disp_aaa_r_x0, sizes, subsizes, starts, aaa_r_x0)
    ...
    call mpiio_write_var_2d(fh_aaa_r_y0, disp_aaa_r_y0, sizes, subsizes, starts, aaa_r_y0)
...
!-----------------------------------------------!
  subroutine loop_io_3D
    ...
    call mpiio_write_var(fh_aaa, disp_psi, sizes, subsizes, starts, aaa)
...
!-----------------------------------------------!
  subroutine save_restart
    ...
    call mpiio_write_one(aaa, sizes, subsizes, starts, trim(restart_dir)//'aaa.dat')
...
!-----------------------------------------------!
  subroutine finish_io
    ...
    call MPI_FILE_CLOSE(fh_aaa,ierr)
    ...
    call MPI_FILE_CLOSE(fh_aaa_r_z0,ierr)
    call MPI_FILE_CLOSE(fh_aaa_r_x0,ierr)
    call MPI_FILE_CLOSE(fh_aaa_r_y0,ierr)
