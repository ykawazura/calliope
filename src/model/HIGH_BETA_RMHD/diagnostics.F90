...
!-----------------------------------------------!
  subroutine loop_diagnostics
    ...
    use params, only: zi, nonlinear, q, kappa_b, kappa_p, &
    ...
    do j = iky_st, iky_en
      do k = ikz_st, ikz_en
        do i = ikx_st, ikx_en
          ...
          p_aw        (i, k, j) = - 2.d0*zi*ky(j)*0.5d0*(phi_mid*conjg(bpa_mid) - bpa_mid*conjg(phi_mid))
          p_compr     (i, k, j) = - zi*ky(j)*( &
                                                                kappa_b*kappa_p *0.5d0*(psi_mid*conjg(upa_mid) - upa_mid*conjg(psi_mid)) &
                                             + (2.d0 - kappa_p*(1.d0 + kappa_b))*0.5d0*(phi_mid*conjg(bpa_mid) - bpa_mid*conjg(phi_mid)) &
                                           )
