...
!-----------------------------------------------!
  subroutine solve
    ...
    use params, only: kappa_b, kappa_p, va2cs2_plus_1, nupe_x , nupe_x_exp , nupe_z , nupe_z_exp , &
        ...
        do k = ikz_st, ikz_en
          do i = ikx_st, ikx_en
            ...
            exp_terms(i,k,j,iomg) = nonlin(i,k,j,iomg) - zi*kz(k)*kprp2(i,k,j)*psi(i,k,j) &
                                         + 2.d0*zi*ky(j)*bpa(i,k,j)
            exp_terms(i,k,j,ipsi) = nonlin(i,k,j,ipsi) + zi*kz(k)*phi(i,k,j)
            exp_terms(i,k,j,iupa) = nonlin(i,k,j,iupa) + zi*kz(k)*bpa(i,k,j) - kappa_b*kappa_p*zi*ky(j)*psi(i,k,j)
            exp_terms(i,k,j,ibpa) = (nonlin(i,k,j,ibpa) + zi*kz(k)*upa(i,k,j) + (kappa_p*(1.d0 + kappa_b) - 2.d0)*zi*ky(j)*phi(i,k,j) ) &
                                         /va2cs2_plus_1
            ! 1st order 
            if(counter == 1) then
...
!-----------------------------------------------!
  subroutine get_nonlinear_terms(phi, psi, upa, bpa, dt_reset)
    ...
    integer :: i, j, k
    real   (r8) :: max_vel_x, max_vel_y, dt_cfl, dt_digit
...
!-----------------------------------------------!
  subroutine get_ext_terms(exp_terms, &
                           ...
                           nonlin, &
                           ky, kz, kprp2)
    use params, only: zi, va2cs2_plus_1, kappa_b, kappa_p
    implicit none
    ...
    exp_terms(iomg) = nonlin(iomg) - zi*kz*kprp2*psi + 2.d0*zi*ky*bpa
    exp_terms(ipsi) = nonlin(ipsi) + zi*kz*phi
    exp_terms(iupa) = nonlin(iupa) + zi*kz*bpa - kappa_b*kappa_p*zi*ky*psi
    exp_terms(ibpa) = ( nonlin(ibpa) + zi*kz*upa + (kappa_p*(1.d0 + kappa_b) - 2.d0)*zi*ky*phi )/va2cs2_plus_1
