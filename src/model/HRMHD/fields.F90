...
!-----------------------------------------------!
  subroutine init_random
    ...
    implicit none
    real   (r8), allocatable, dimension(:,:,:) :: phi_r, psi_r, upa_r, bpa_r
    ...
    allocate(upa_r(ily_st:ily_en, ilz_st:ilz_en, ilx_st:ilx_en), source=0.d0)
    allocate(bpa_r(ily_st:ily_en, ilz_st:ilz_en, ilx_st:ilx_en), source=0.d0)
    ...
    call random_number(upa_r)
    call random_number(bpa_r)
    !^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^!
    ...
    call p3dfft_ftran_r2c(upa_r*1d-1, upa, 'fft'); upa = upa/nlx/nly/nlz 
    call p3dfft_ftran_r2c(bpa_r*1d-1, bpa, 'fft'); bpa = bpa/nlx/nly/nlz 
      ...
      do k = ikz_st, ikz_en
        do i = ikx_st, ikx_en
          if (kx(i)**2 + ky(j)**2 + kz(k)**2 /= 0.d0) then
            ...
            upa(i, k, j) = upa(i, k, j)/dsqrt(kx(i)**2 + ky(j)**2 + kz(k)**2)
            bpa(i, k, j) = bpa(i, k, j)/dsqrt(kx(i)**2 + ky(j)**2 + kz(k)**2)
          endif
...
!-----------------------------------------------!
  subroutine init_OT2
    ...
    call p3dfft_ftran_r2c(upa_r, upa, 'fft'); upa = upa/nlx/nly/nlz
    call p3dfft_ftran_r2c(bpa_r, bpa, 'fft'); bpa = bpa/nlx/nly/nlz
    do j = iky_st, iky_en
      do k = ikz_st, ikz_en
        do i = ikx_st, ikx_en
          omg(i, k, j) = phi(i, k, j)*(-kprp2(i, k, j))
...
!-----------------------------------------------!
  subroutine init_OT3
    ...
    call p3dfft_ftran_r2c(phi_r, phi, 'fft'); phi = phi/nlx/nly/nlz
    call p3dfft_ftran_r2c(psi_r, psi, 'fft'); psi = psi/nlx/nly/nlz
    call p3dfft_ftran_r2c(upa_r, psi, 'fft'); upa = upa/nlx/nly/nlz
    call p3dfft_ftran_r2c(bpa_r, psi, 'fft'); bpa = bpa/nlx/nly/nlz
    do j = iky_st, iky_en
      do k = ikz_st, ikz_en
        do i = ikx_st, ikx_en
          omg(i, k, j) = phi(i, k, j)*(-kprp2(i, k, j))
